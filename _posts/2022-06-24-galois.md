...
For intuitionistic logic (using Kripke frame), let W be the set of possible words, $\sqsubseteq$ be the accessibility relation between worlds. $\lagnel W,  \sqsubseteq \rangle$ is a poset.

Inclusion posets:

Any set A, $\langle \mathcal {P}(A), \subseteq$ is a poset.
