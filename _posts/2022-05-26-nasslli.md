...
higher order logic 4 steps： 0: Types 1:complex predicates 把 lambda notation 和 first-order predicate 结合起来 (Extensional Beta/Beta 区别是 Beta 可以 go as deep as it wants to substitute easily) 3: higher-order predicates 这样first-order quantifiers can be seen as higher-order predicates eg. $\forall (\lambda v. P)$ 
可以把 $\forall$ 
当作 
$(e \to t) \to t$ 
先把
...
4 complex higher-order predicates $(\lambda v:\sigma . A):\sigma \to \tau$
5 higher-order quantifiers and identity $(\forall_\sigma :(\sigma \to t)\to t)$ and formulate Hibert-style axiomatic systems to see how they behave.（先不用看 model theoretic results and see how we can interpret them）
